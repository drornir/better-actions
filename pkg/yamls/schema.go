// Package yamls is used to parse github YAMLs with extensions.
// It is adapted from nextos/act/pkg/schema
package yamls
...
var (
	workflowSchemaInstance *Schema
	actionSchemaInstance   *Schema
)

func init() {
	workflowSchemaInstance = &Schema{}
	if err := json.Unmarshal([]byte(workflowSchema), workflowSchemaInstance); err != nil {
		panic(fmt.Sprintf("failed to unmarshal workflow schema: %v", err))
	}

	actionSchemaInstance = &Schema{}
	if err := json.Unmarshal([]byte(actionSchema), actionSchemaInstance); err != nil {
		panic(fmt.Sprintf("failed to unmarshal action schema: %v", err))
	}
}

func GetWorkflowSchema() *Schema {
	return workflowSchemaInstance
}

func GetActionSchema() *Schema {
	return actionSchemaInstance
}
