...
func ReadAction(in io.Reader) (*Action, error) {
	...
	if err != nil {
		return nil, oops.Wrapf(err, "failed to decode action YAML")
	}
...
func (a *Action) UnmarshalYAML(node *yaml.Node) error {
	...
	if err := (&Node{
		...
		return oops.
			With("definition", "action-root").
			Wrapf(err, "action YAML schema validation error")
	}
...
func (a *ActionRunsUsing) UnmarshalYAML(unmarshal func(any) error) error {
	...
	if err := unmarshal(&using); err != nil {
		return oops.Wrapf(err, "failed to unmarshal action runs using")
	}
	...
	default:
		return oops.
			...
			Errorf("the runs.using key in action.yml must be one of the allowed values, got %s", format)
	}
	return nil
}
