...
func (w *Workflow) UnmarshalYAML(node *yaml.Node) error {
	...
	if err := (&Node{
		...
		return oops.
			With("definition", "workflow-root").
			Wrapf(err, "actions YAML schema validation error")
	}
...
func (w *WorkflowStrict) UnmarshalYAML(node *yaml.Node) error {
	...
	if err := (&Node{
		...
		return oops.
			With("definition", "workflow-root-strict").
			Wrapf(err, "actions YAML strict schema validation error detected:")
	}
	type WorkflowDefault Workflow
	return node.Decode((*WorkflowDefault)(w))
}
...
func (w *Workflow) On(ctx context.Context) ([]string, error) {
	switch w.RawOn.Kind {
	case yaml.ScalarNode:
		var val string
		err := w.RawOn.Decode(&val)
		if err != nil {
			return nil, fmt.Errorf("failed to decode RawOn scalar node: %w", err)
		}
	...
	case yaml.SequenceNode:
		...
		err := w.RawOn.Decode(&val)
		if err != nil {
			return nil, fmt.Errorf("failed to decode RawOn sequence node: %w", err)
		}
		return val, nil
	case yaml.MappingNode:
		...
		err := w.RawOn.Decode(&val)
		if err != nil {
			return nil, fmt.Errorf("failed to decode RawOn mapping node: %w", err)
		}
		var keys []string
		for k := range val {
			keys = append(keys, k)
...
// Defaults are settings that will apply to all steps in the job or workflow
type Defaults struct {
	Run RunDefaults `yaml:"run"`
}

// RunDefaults are default for run type steps
...
func (s Strategy) GetMaxParallel(ctx context.Context) int {
	...
	if s.MaxParallelString != "" {
		...
		if maxParallel, err = strconv.Atoi(s.MaxParallelString); err != nil {
			logger.E(ctx, "failed to parse 'max-parallel' option: %v", err)
		}
	}
	return maxParallel
}
...
func (s Strategy) GetFailFast(ctx context.Context) bool {
	logger := log.FromContext(ctx)
	...
	failFast := true
	logger.D(ctx, "FailFastString: %s", s.FailFastString)
	if s.FailFastString != "" {
		var err error
		if failFast, err = strconv.ParseBool(s.FailFastString); err != nil {
			logger.E(ctx, "failed to parse 'fail-fast' option: %v", err)
		}
	}
	return failFast
}
...
func (j *Job) GetMatrixes(ctx context.Context) ([]map[string]any, error) {
	logger := log.FromContext(ctx)
	matrixes := make([]map[string]any, 0)
	if j.Strategy != nil {
		j.Strategy.FailFast = j.Strategy.GetFailFast(ctx)
		j.Strategy.MaxParallel = j.Strategy.GetMaxParallel(ctx)
			...
			for _, e := range m["exclude"] {
				...
				for k := range e {
					if _, ok := m[k]; ok {
						...
						return nil, oops.
							With("exclude_key", k).
							Errorf("the workflow is not valid. Matrix exclude key %q does not match any key within the matrix", k)
					}
				}
			}
			delete(m, "exclude")
		...
		MATRIX:
			for _, matrix := range matrixProduct {
				for _, exclude := range excludes {
					if commonKeysMatch(matrix, exclude) {
						logger.D(ctx, "skipping matrix '%v' due to exclude '%v'", matrix, exclude)
						continue MATRIX
					}
				}
			...
			for _, include := range includes {
				...
				for _, matrix := range matrixes {
					if commonKeysMatch2(matrix, include, m) {
						matched = true
						logger.D(ctx, "adding include values '%v' to existing entry", include)
			...
			for _, include := range extraIncludes {
				logger.D(ctx, "adding include '%v'", include)
		...
		matrixes = append(matrixes, make(map[string]any))
		logger.D(ctx, "empty Strategy, matrixes=%v", matrixes)
	}
	return matrixes, nil
}
...
func (j *Job) Type() (JobType, error) {
	...
	if isReusable {
		...
		return JobTypeInvalid, oops.
			With("uses_path", j.Uses).
			With("job_name", j.Name).
			Errorf("`uses` key references invalid workflow path '%s'. must start with './' if it's a local workflow, or must start with '<org>/<repo>/' and include an '@' if it's a remote workflow", j.Uses)
	}
