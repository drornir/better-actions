...
- [x] In `StepRun.Run`:
    ...
    - [x] If `step.WorkingDirectory` is explicitly set in YAML, use it (resolved relative to workspace?). *Refinement: GitHub Actions defaults to GITHUB_WORKSPACE. If `working-directory` is relative, it's relative to GITHUB_WORKSPACE.*
